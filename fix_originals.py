...
ORIGINALS_DIR = r"c:\xampp\htdocs\pakgusu-1\intoriza\pakgusu-new\intoriza\extra\originals"
...
def fix_original_file(filename):
    print(f"Processing: {filename}")
    file_path = os.path.join(ORIGINALS_DIR, filename)
    
    # Original files are 2 levels deep from intoriza root (extra/originals/)
    prefix = "../../"
    ...
    # Fix all asset links to use ../../ prefix
    # Fix link tags (CSS)
...
def main():
    print("=== Fixing Original Template Files ===\n")
    
    for filename in FILES_TO_PROCESS:
        file_path = os.path.join(ORIGINALS_DIR, filename)
        if os.path.exists(file_path):
            fix_original_file(filename)
        else:
